...
function isOnline(home_url,passport_url){
	...
	$.getJSON( "/ajax/isOnline/?callback=?", function( data ) {
		if (data.welcome){
			...
			$('#login').html('<span class="logininfo"> '+data.welcome+'</span> <a href="'+passport_url+'/impl/logout/?returnUrl='+passport_url+'/">[退出]</a>');
		}else{
		...
		if(data.qqcb){
			$('#login').html('<span class="logininfo"> '+data.welcome+'</span> <a href="'+home_url+'/impl/logout/?returnUrl='+passport_url+'/">[退出]</a>');
			$('#qqcb').html(data.qqcb); 
		}
	});
}
